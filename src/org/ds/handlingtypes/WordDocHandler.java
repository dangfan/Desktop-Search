...
/**
 * 处理Microsoft Word 97-2003文件
 */
public class WordDocHandler implements FileHandler
{
            ...
            throws FileHandlerException
    {
        ...
        try
        {
            bodyText = new WordExtractor(poifsfs).getText();
        }
        catch (IOException e)
        {
            throw new FileHandlerException(
        ...
        try
        {
            summary = new HWPFDocument(poifsfs).getSummaryInformation();
        }
        catch (Exception e)
        {
        ...
        doc.add(new Field("date", Utility.getFormattedDate(
                summary.getLastSaveDateTime()), Field.Store.YES, Field.Index.NO));
        //添加文件路径
